...
digitizer_t digitizer_driver_get_report(digitizer_t digitizer_report) {
    ...
    if (status == I2C_STATUS_SUCCESS) {
        for (int i = 0; i < 5; i++) {
            digitizer_report.fingers[i].x = AZOTEQ_IQS5XX_COMBINE_H_L_BYTES(digitizer_data.fingers[i].x.h, digitizer_data.fingers[i].x.l);
            digitizer_report.fingers[i].y = AZOTEQ_IQS5XX_COMBINE_H_L_BYTES(digitizer_data.fingers[i].y.h, digitizer_data.fingers[i].y.l);

            // TODO: These thresholds were plucked out of thin air.
            digitizer_report.fingers[i].tip = digitizer_data.fingers[i].touch_area != 0;
            digitizer_report.fingers[i].confidence = digitizer_data.fingers[i].touch_area < 50;
        }
        azoteq_iqs5xx_end_session();
    }
